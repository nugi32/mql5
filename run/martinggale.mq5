...
void OnTick()
{
   ...
   //=== 1) EMERGENCY EQUITY PROTECTION (checked every tick, highest priority) ===
   if(count > 0)
   {
   ...
   //=== 2) PROFIT-ONLY BASKET EXIT (checked every tick) ========================
   ...
   //=== 3) GRID MARTINGALE ADD-ON (checked every tick) =========================
   ...
   //=== 4) FRESH GRID LEVEL 1 ENTRY (only once per new bar, only if flat) =======
   if(count == 0 && IsNewBar())
   {
      if(CheckEntrySignal())
