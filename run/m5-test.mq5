...
//=== INPUT PARAMETERS ===============================================
input group "── Trade Settings ──" input double InpLotSize = 0.1; // Lot Size
input double SlPoints = 10;
input int InpExitCandles = 4;        // Exit after N candles (signal timing ≈ 3.62)
input ulong InpMagicNumber = 202401; // Magic Number

input group "── RSI Settings ──" input int InpRSIPeriod = 14; // RSI Period
input double InpRSIDeepLevel = 25.0;                          // RSI Deep Oversold Threshold (<)

input group "── ADX Settings ──" input int InpADXPeriod = 14; // ADX Period
input double InpADXLevel = 25.0;                              // ADX Trending Threshold (>)

input group "── EMA Settings ──" input int InpEMA21Period = 21; // EMA Fast Period
input int InpEMA50Period = 50;                                  // EMA Mid Period
input int InpEMA100Period = 100;                                // EMA Slow Period

input group "── MACD Settings ──" input int InpMACDFast = 12; // MACD Fast EMA
                    ...
                    double &macd, double &sig)
{
    ...
    // EMAs — buffer 0
    if (CopyBuffer(hEMA21, 0, 1, 1, buf) < 1)
        return false;
    ema21 = buf[0];
    if (CopyBuffer(hEMA50, 0, 1, 1, buf) < 1)
        return false;
    ema50 = buf[0];
    if (CopyBuffer(hEMA100, 0, 1, 1, buf) < 1)
        return false;
    ema100 = buf[0];

    // MACD — buffer 0 = MACD line, buffer 1 = Signal line
    if (CopyBuffer(hMACD, 0, 1, 1, buf) < 1)
        return false;
    macd = buf[0];
    if (CopyBuffer(hMACD, 1, 1, 1, buf) < 1)
        return false;
    sig = buf[0];
...
bool OpenBuy()
{
    ...
    req.price = ask;
    req.sl = NormalizeDouble(ask - SlPoints * _Point, _Digits);
    req.deviation = 20;
...
void OnTick()
{
    ...
    //--- jika ada posisi terbuka → update candle counter
    if(HasOpenPosition())
    {
        g_candleCount++;
        ...
        return; // jangan entry saat posisi masih ada
    }

    // reset kalau tidak ada posisi
    g_candleCount = 0;
