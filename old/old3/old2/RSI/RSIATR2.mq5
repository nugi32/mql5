...
//================ GLOBAL =================
int    atrHandle, emaLayerHandle, rsiHandle;
double ATR[], emaLayer[], RSI[];
...
int OnInit()
{
   ...
   if(atrHandle==INVALID_HANDLE || emaLayerHandle==INVALID_HANDLE || rsiHandle == INVALID_HANDLE)
      return INIT_FAILED;
...
bool rsiSell = RSI[1] >= overPrice;
   ...
   // ===== FINAL SIGNAL =====
   if(rsiBuy && HigherTF_Bullish())
      OpenBuy();

   if(rsiSell && HigherTF_Bearish())
      OpenSell();
}
