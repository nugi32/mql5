...
//+------------------------------------------------------------------+
//| Calculate lot size based on risk and check margin               |
//+------------------------------------------------------------------+
double CalculateLotSize(double slDistance)
{
   ...
   // Cek margin requirement
   double required_margin;
   double price = SymbolInfoDouble(_Symbol, SYMBOL_ASK); // Default untuk Buy
   
   // Coba hitung margin untuk Buy dan Sell, ambil yang terbesar
   if(!OrderCalcMargin(ORDER_TYPE_BUY, _Symbol, lotSize, price, required_margin))
   {
   ...
   // Jika margin tidak cukup, kurangi lot size
   if(required_margin > free_margin)
   {
      double safety_margin = free_margin * 0.9; // Gunakan 90% dari free margin
      lotSize = lotSize * (safety_margin / required_margin);
      
      // Re-normalize setelah penyesuaian
      lotSize = MathFloor(lotSize / lotStep) * lotStep;
      lotSize = MathMax(minLot, MathMin(maxLot, lotSize));
      
      Print("Lot size reduced due to margin constraints. New lot: ", lotSize);
   }
...
//+------------------------------------------------------------------+
//| Display margin information for debugging                         |
//+------------------------------------------------------------------+
void DisplayMarginInfo(double lotSize, ENUM_ORDER_TYPE type)
{
   ...
   double margin;
   if(OrderCalcMargin(type, _Symbol, lotSize, price, margin))
   {
      double free_margin = AccountInfoDouble(ACCOUNT_MARGIN_FREE);
      ...
      Print("Required Margin for ", EnumToString(type), ": ", margin);
      Print("Margin ratio: ", (margin/free_margin)*100, "%");
      Print("Leverage: 1:", AccountInfoInteger(ACCOUNT_LEVERAGE));
      
      if(margin > free_margin)
         Print("⚠️ INSUFFICIENT MARGIN! Required: ", margin, " > Free: ", free_margin);
      else
...
bool CheckMoneyForTrade(string symb, double lots, ENUM_ORDER_TYPE type)
{
   ...
   // Tampilkan info margin untuk debugging
   Print("CheckMoneyForTrade - Required: ", margin, " Free: ", free_margin);
...
void ExecuteBuy(double slDistance, double tpDistance)
{
   ...
   {
      Print("CheckMoneyForTrade FAILED for Buy order");
      return;
   }
...
void ExecuteSell(double slDistance, double tpDistance)
{
   ...
   {
      Print("CheckMoneyForTrade FAILED for Sell order");
      return;
   }
