...
//================ GLOBAL =================
int    emaFastHandle, emaSlowHandle, emaLayerHandle;
double emaFast[], emaSlow[], emaLayer[];
...
int OnInit()
{
   ...
   if(emaFastHandle==INVALID_HANDLE || emaSlowHandle==INVALID_HANDLE || emaLayerHandle==INVALID_HANDLE)
      return INIT_FAILED;
...
void CheckForEntry()
{
   ...
   if(buySignal && HigherTF_Bullish())
      OpenBuy();

   if(sellSignal && HigherTF_Bearish())
      OpenSell();
}
//+------------------------------------------------------------------+
...
void OpenBuy()
{
    ...
    // Hitung Take Profit agar Risk:Reward = 1:1
    double risk = MathAbs(entry - sl);  // jarak SL
    double tp = entry + risk;           // untuk buy, TP di atas entry
...
void OpenSell()
{
    ...
    // Hitung Take Profit agar Risk:Reward = 1:1
    double risk = MathAbs(entry - sl);  // jarak SL
    double tp = entry - risk;           // untuk sell, TP di bawah entry
