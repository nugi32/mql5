...
input bool use_ATR = false;//t
input bool use_EMA = false;//t
input bool use_RSI = false;//


input group "=== TP Settings ==="
input bool UseATR = false;//
input bool UseEMA = false;//t
...
//================ GLOBAL =================//
int    atrHandle, rsiHandle, emaSingleHandle, adxHandle, volHandle;
double ATR[], RSI[], emaSingle[], ADX[], VOL[];
MqlRates rates[];
...
int OnInit()
{
   ...
   emaSingleHandle = iMA(_Symbol,_Period,EMA_Fast_Period,0,MODE_EMA,PRICE_CLOSE);
   adxHandle = iADX(_Symbol, _Period, ADX_Period);
   volHandle = iVolumes(_Symbol, _Period, VOLUME_TICK);

   if(atrHandle==INVALID_HANDLE || rsiHandle == INVALID_HANDLE || emaSingleHandle==INVALID_HANDLE  || adxHandle==INVALID_HANDLE || volHandle==INVALID_HANDLE)
      return INIT_FAILED;

ArraySetAsSeries(ATR,true);         
ArraySetAsSeries(RSI,true);    
ArraySetAsSeries(emaSingle,true);
...
void OnDeinit(const int reason)
{
   ...
   IndicatorRelease(emaSingleHandle);
...
void OnTick()
{
   ...
   if(CopyBuffer(emaSingleHandle,0,0,3,emaSingle) < 3 ) return;
...
void ManageReverseTP(ulong ticket)
{
   ...
   double curentPrice = iClose(_Symbol,_Period,0);
   
   long type = PositionGetInteger(POSITION_TYPE);

   if(type==POSITION_TYPE_BUY &&
      curentPrice < emaSingle[1])
...
bool emaBuy()
{
   ...
   double ema = emaSingle[1];
   return rates[1].low <= ema && rates[1].close > ema;
}
...
bool emaSell()
{
   if(!useEMA) return true;
   double ema = emaSingle[1];
   return rates[1].high >= ema && rates[1].close < ema;
}
