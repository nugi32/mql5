...
//================ GLOBAL =================
int    emaHandle, emaLayerHandle, adxHandle;
double   ema[], emaLayer[], ADX[];
MqlRates rates[];
...
int OnInit()
{
   ...
   if(emaHandle==INVALID_HANDLE || emaLayerHandle==INVALID_HANDLE || adxHandle==INVALID_HANDLE)
      return INIT_FAILED;
...
void CheckForEntry()
{
   ...
   if(buySignal && adxTrending() && HigherTF_Bullish())
      OpenBuy();

   if(sellSignal && adxTrending() && HigherTF_Bearish())
      OpenSell();
}
