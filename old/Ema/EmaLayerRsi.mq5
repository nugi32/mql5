...
input bool Use_MTF_Filter = true;
...
//================ GLOBAL =================
int    emaFastHandle, emaSlowHandle, emaLayerHandle, rsiHandle;
double emaFast[], emaSlow[], emaLayer[], RSI[];
...
int OnInit()
{
   ...
   if(emaFastHandle==INVALID_HANDLE || emaSlowHandle==INVALID_HANDLE || emaLayerHandle==INVALID_HANDLE || rsiHandle == INVALID_HANDLE)
      return INIT_FAILED;
...
void CheckForEntry()
{
   ...
   // ===== EMA CROSS =====
   bool buyCross  = emaFast[1] > emaSlow[1] && emaFast[2] <= emaSlow[2];
   bool sellCross = emaFast[1] < emaSlow[1] && emaFast[2] >= emaSlow[2];

   // ===== RSI CONFIRMATION =====
   /*
   bool rsiBuy  = RSI[1] > overSold && RSI[1] < overPrice;
   bool rsiSell = RSI[1] < overPrice && RSI[1] > overSold;
*/
...
bool rsiSell = RSI[1] < overPrice && RSI[1] > 50;
   ...
   // ===== FINAL SIGNAL =====
   if(buyCross && rsiBuy && HigherTF_Bullish())
      OpenBuy();

   if(sellCross && rsiSell && HigherTF_Bearish())
      OpenSell();
}
