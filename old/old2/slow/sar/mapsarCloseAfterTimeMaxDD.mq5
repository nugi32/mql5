...
input bool useCloseAfterXMinutes = true;


input double MaxRiskPercent = 2.0; // risiko maksimal dalam persen
input int maxMinutes = 60; // tutup posisi setelah X menit
//+------------------------------------------------------------------+
//| Global                                                           |
//+------------------------------------------------------------------+
CExpert     ExtExpert;
...
//+------------------------------------------------------------------+
bool ClosePositionAfterXMinutes()
{
    if (!useCloseAfterXMinutes) return false;
   // --- Only allow 1 position
   if(PositionsTotal() != 1)
      return false;
