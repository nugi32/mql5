...
/*
//+------------------------------------------------------------------+
//|              ATR Mean Reversion EA (No iCustom)                  |
//+------------------------------------------------------------------+
#property strict
...
int OnInit()
{
   ...
   if(atrHandle == INVALID_HANDLE || maHandle == INVALID_HANDLE ||
      fastHandle == INVALID_HANDLE || slowHandle == INVALID_HANDLE)
   {
      Print("Indicator init failed: ", GetLastError());
      return(INIT_FAILED);
   }
...
//+------------------------------------------------------------------+
//| SIDEWAYS DETECTOR (INLINE VERSION)                              |
//+------------------------------------------------------------------+
bool isSideways()
{
   int neededBars = LookbackCandles + 2;
   ...
   ArrayResize(fastEMA, neededBars);
   ArrayResize(slowEMA, neededBars);

   if(CopyBuffer(fastHandle, 0, 0, neededBars, fastEMA) <= 0)
      return false;

   if(CopyBuffer(slowHandle, 0, 0, neededBars, slowEMA) <= 0)
      return false;
   ...
   {
      bool prevAbove = fastEMA[i] > slowEMA[i];
      bool currAbove = fastEMA[i-1] > slowEMA[i-1];

      if(prevAbove != currAbove)
         crossCount++;
   }

   return (crossCount >= CrossThreshold);
}
//+------------------------------------------------------------------+
void tradeBuy(double sl, double tp)
{
